...
import {
  View, Text, FlatList, TouchableOpacity, Alert, StyleSheet, Modal, ScrollView
} from 'react-native';
            ...
            {item.itens.map((subitem, idx) => (
              <View key={idx}>
                <Text style={styles.sub}>
                  {subitem.quantidade}x {subitem.produto?.nome || subitem.produto?.codigo || 'Produto manual'}
                </Text>
                {subitem.observacao ? <Text style={styles.obs}>Obs: {subitem.observacao}</Text> : null}
              </View>
            ))}
            <Text style={styles.sub}>Forma: {item.formaPagamento || 'N/A'}</Text>
            <Text style={styles.sub}>Total: R$ {total.toFixed(2)}</Text>
            {item.formaPagamento === 'dinheiro' && item.valorTroco && (
              <>
  ...
  return (
    <View style={styles.container}>
      <FlatList
        ...
        ListEmptyComponent={<Text style={styles.emptyText}>Nenhum pedido registrado</Text>}
      />
      ...
      <Modal visible={modalEntregaVisible} transparent animationType="slide">
        <View style={{ flex: 1, backgroundColor: 'rgba(0,0,0,0.5)', justifyContent: 'center', alignItems: 'center' }}>
          <View style={{ width: '85%', backgroundColor: 'white', padding: 20, borderRadius: 12 }}>
...
const styles = StyleSheet.create({
  ...
  listContainer: { paddingBottom: 120, paddingTop: 16, paddingHorizontal: 16 },
  card: {
    backgroundColor: '#fff', borderRadius: 12, paddingVertical: 16, paddingHorizontal: 12,
    marginBottom: 12, shadowColor: '#000', shadowOpacity: 0.1, shadowRadius: 6, elevation: 3,
    position: 'relative',
  },
  statusTag: {
    position: 'absolute', left: 0, top: 0, bottom: 0,
    width: 10, borderTopLeftRadius: 12, borderBottomLeftRadius: 12,
  },
  ...
  actionsContainerRow: { justifyContent: 'center', alignItems: 'center', gap: 8 },
  statusButtons: { flexDirection: 'row', gap: 8, marginBottom: 4 },
  statusDot: { width: 12, height: 12, borderRadius: 6 },
  horaPedido: {
    position: 'absolute', right: 12, top: 8, fontSize: 12, color: '#999', fontStyle: 'italic'
  },
  emptyText: { textAlign: 'center', color: '#888', marginTop: 40 },
  addButton: {
    position: 'absolute', bottom: 70, right: 20, backgroundColor: '#2196F3',
